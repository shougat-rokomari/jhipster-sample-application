...
type RestOf<T extends IPerson | NewPerson> = Omit<T, 'dateOfBirth' | 'dateOfDeath'> & {
...
export class PersonService {
  ...
  protected convertDateFromClient<T extends IPerson | NewPerson | PartialUpdatePerson>(person: T): RestOf<T> {
    return {
      ...person,
      dateOfBirth: person.dateOfBirth?.toJSON() ?? null,
      dateOfDeath: person.dateOfDeath?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPerson: RestPerson): IPerson {
    return {
      ...
      dateOfDeath: restPerson.dateOfDeath ? dayjs(restPerson.dateOfDeath) : undefined,
    };
  }
